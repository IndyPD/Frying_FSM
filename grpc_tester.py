...
def run_dummy_client(ip, port):
    ...
    try:
        ...
        while True:
            # 사용자로부터 명령 입력 받기
            print("명령어는 다음과 같습니다:")
            print("1:set_int, 2:get_int, 3:set_ints, 4:get_ints")
            print("5:set_string_with_id, 6:get_string_with_id")
            print("7:send_recipe_json")
            print("8:set_float, 9:get_float, 10:set_floats, 11:get_floats")
            ...
            # 7번 명령어를 입력하면 레시피 JSON을 자동으로 전송
                ...
                try:
                    user_input = input("인덱스(정수), 값(실수)을 콤마(,)로 구분해서 입력하세요 (예: 1,3.14): ")
                    parts = user_input.split(',')
                    idx = int(parts[0].strip())
                    val = float(parts[1].strip())
                    request = template_pb2.GFloat(idx=idx, val=val)
                    stub.SetFloat(request)
                    print(f"서버에 인덱스 {idx}, float 값 {val} 전달 완료.")
                except (ValueError, grpc.RpcError) as e:
                    print(f"오류 발생: {e}")
            ...
            elif command == '10': # set_floats
                try:
                    user_input = input("시작 인덱스(정수)와 float 값들(콤마로 구분)을 콤마(,)로 구분해서 입력하세요 (예: 1,1.1,2.2,3.3): ")
                    parts = user_input.split(',')
                    idx = int(parts[0].strip())
                    vals = [float(x.strip()) for x in parts[1:]]
                    request = template_pb2.GFloats(idx=idx, val=vals)
                    stub.SetFloats(request)
                    print(f"서버에 인덱스 {idx}부터 float 값 {vals} 전달 완료.")
                except (ValueError, grpc.RpcError) as e:
                    print(f"오류 발생: {e}")

            elif command == '11': # get_floats
                try:
                    user_input = input("시작 인덱스(정수)와 가져올 개수(정수)를 콤마(,)로 구분해서 입력하세요 (예: 1,5): ")
                    idx, count = map(int, user_input.split(','))
                    request = template_pb2.GInt(idx=idx, val=count)
                    response = stub.GetFloats(request)
                    print(f"서버로부터 받은 float 값들: {list(response.val)}")
                except (ValueError, grpc.RpcError) as e:
                    print(f"오류 발생: {e}")
