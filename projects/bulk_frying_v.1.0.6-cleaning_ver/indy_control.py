        ...
        try:
            ...
            if cleaning_mode_command != 0:
                ...
                if cleaning_mode_command == 2:  # 청소 모드
                    ...
                    Logger.info("[Safety Mode] 청소 모드가 활성화되었습니다. 라이다 센서 검사를 무시합니다.")
                elif cleaning_mode_command == 1:  # 일반 모드
                    self.indy.set_do([{'address': self.cleaning_mode_do_channel, 'state': 0}])
                    Logger.info("[Safety Mode] 일반 모드가 활성화되었습니다. 라이다 센서 검사를 시작합니다.")
            ...
            # 1순위: 보호 영역 (di2) - ✨ 프로그램 일시정지
            if is_di2_on:
                bb.set("ui/state/lidar_sensor", 2)
                if self.program_state == ProgramState.PROG_RUNNING: # 실행 중일 때만 일시정지
                    try:
                        Logger.info("라이다 보호 영역(DI2) 침범. 프로그램을 일시정지합니다.")
                        self.indy.pause_program() # pause_program으로 변경
                    except Exception as e:
                        Logger.error(f"프로그램 일시정지 실패: {e}")
