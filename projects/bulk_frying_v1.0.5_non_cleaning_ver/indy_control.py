...
#             # UI에서 청소 모드 명령 확인
#             cleaning_mode_command = bb.get('ui/command/cleaning_mode')  # 0: 대기, 1: 일반모드, 2: 청소모드
            
#             # UI 명령에 따라 상태 및 DO 신호 직접 설정
...
#                 bb.set('ui/state/cleaning_mode', 0)
#                 self.indy.set_do([{'address': self.cleaning_mode_do_channel, 'state': 0}])
...
#             # 실제 청소 모드 상태 확인 (UI 명령과 DO 신호가 모두 참일 때)
#             is_cleaning_mode = (cleaning_mode_command == 2) and (self.get_dio_channel(do, self.cleaning_mode_do_channel) == 1)
...
#                     Logger.info("Lidar detected object in cleaning mode, but no action will be taken.")
#             elif cleaning_mode_command == 1:
