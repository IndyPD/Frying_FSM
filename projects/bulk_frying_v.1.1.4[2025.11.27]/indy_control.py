...
#         self.lidar_di1_paused = False
#         self.lidar_restart_latch = False
#         self.last_lidar_restart_state = 0
...
#         - DI1 (경고 영역): 로봇 속도를 0으로 감속 후 UI에 팝업. DO 11, 12 점멸.
...
#             current_lidar_restart_state = bb.get("ui/command/lidar_restart")
#             if current_lidar_restart_state == 1 and self.last_lidar_restart_state == 0:
#                 # RISING EDGE DETECTED
#                 Logger.info("Lidar restart button pressed.")
#                 if not is_di1_on:
#                     # Safe to restart
#                     bb.set("ui/state/lidar_popup", 0)
#                     self.indy.set_speed_ratio(100)
#                     self.lidar_di1_paused = False
#                     Logger.info("Lidar restart approved.")
#                     # Now, we need to tell the UI to reset the command.
#                     bb.set("ui/reset/lidar_restart", True)
#                 else:
#                     # Not safe
#                     Logger.warn("Lidar restart command received, but area is not safe.")
#                     # We should still reset the command, otherwise it will trigger again
#                     bb.set("ui/reset/lidar_restart", True)

#             self.last_lidar_restart_state = current_lidar_restart_state
...
#                 ])
#                 if self.program_state == ProgramState.PROG_RUNNING:
#                     if not self.lidar_di1_paused:
#                         try:
#                             Logger.info("라이다 경고 영역(DI1) 침범. 로봇을 정지합니다.")
#                             # self.indy.stop_program()
#                             self.indy.set_speed_ratio(0)
#                             bb.set("ui/state/lidar_popup", 1)
#                             self.lidar_di1_paused = True
#                             # self.indy.set_speed_ratio(100)
#                         except Exception as e:
#                             Logger.error(f"프로그램 정지 실패: {e}")
#                 return
...
#                             Logger.info("라이다 주의 영역(DI0) 침범. 속도를 30으로 변경합니다.")
#                 return
#             # 3순위: 주의 영역 (di0) - 속도 감속
#             # elif is_di0_on:
#             #     self.indy.set_do([
...
class RobotCommunication:
    def __init__(self, config_file="projects/bulk_frying/configs/indy_command.json", *args, **kwargs):
        ...
        self.beep_state = DigitalState.OFF_STATE
    ...
    def handle_lidar_safety(self):
        """
        Lidar 센서 기반 안전 로직 (수정됨)
        
        시나리오:
        1. 로봇이 정지 구역(DI1)에 있을 때: 사용자가 '재시작'을 여러 번 눌러도 로봇은 움직이지 않음
        2. 로봇이 안전 구역으로 나왔을 때: 자동으로 재시작되지 않고, 멈춘 상태를 그대로 유지
        3. 안전 구역에 있는 상태에서 사용자가 '재시작'을 눌렀을 때: 그제서야 비로소 로봇이 다시 움직임
        
        - DI2 (보호 영역): 로봇 프로그램을 즉시 정지. DO 12, 13 점멸.
        - DI1 (경고 영역): 로봇 속도를 0으로 감속 후 UI에 팝업. DO 11, 12 점멸.
        - DI0 (주의 영역): 로봇 속도를 30으로 감속. DO 11, 12 점멸.
        - 안전 영역: DO 10 점멸.
        - 센서 우선순위: DI2 > DI1 > DI0 순서로 적용.
        """
        # 1초마다 점멸 상태를 업데이트
        ...
        if current_time - self.last_beep_time >= 1.0:
            self.beep_state = 1 - self.beep_state
            self.last_beep_time = current_time

        try:
            # 시뮬레이션 모드와 실제 모드에 따라 DI/DO 신호 소스를 선택
            if self.is_sim_mode:
                io_signals = self.indy.get_do()['signals']
            else:
                io_signals = self.indy.get_di()['signals']

            is_di0_on = self.get_dio_channel(io_signals, self.lidar_warning_channel) == 1
            is_di1_on = self.get_dio_channel(io_signals, self.lidar_protective_channel) == 1
            is_di2_on = self.get_dio_channel(io_signals, self.lidar_ossd_channel) == 1
            ...
            if current_lidar_restart_state == 1 and self.last_lidar_restart_state == 0:
                Logger.info("Lidar restart button pressed (rising-edge detected).")
                
                if not is_di1_on and not is_di0_on:
                    Logger.info("✅ Restart approved - Robot is in SAFE zone. Resuming operation.")
                    bb.set("ui/state/lidar_popup", 0)
                    self.indy.set_speed_ratio(100) # 복원: 재시작 시 속도 100으로 설정
                    self.lidar_di1_paused = False
                elif is_di1_on:
                    Logger.warn("❌ Restart denied - Robot still in PROTECTIVE zone (DI1 active).")
                elif is_di0_on:
                    Logger.warn("❌ Restart denied - Robot still in WARNING zone (DI0 active).")
            ...
            # UI에 현재 라이다 센서 상태 업데이트
            if is_di2_on: bb.set("ui/state/lidar_sensor", 2)
            elif is_di1_on: bb.set("ui/state/lidar_sensor", 1)
            elif is_di0_on: bb.set("ui/state/lidar_sensor", 0)
            else: bb.set("ui/state/lidar_sensor", 3)

            # --- 센서 우선순위에 따른 로봇 제어 ---

            # 1순위: 보호 영역 (DI2) - 즉시 정지 (OSSD)
            if is_di2_on:
                self.indy.set_do([
                    {'address': 10, 'state': self.blinking_light_state}, # Red blinking
                    {'address': 11, 'state': DigitalState.OFF_STATE},
                    {'address': 12, 'state': DigitalState.OFF_STATE},
                    {'address': 13, 'state': self.beep_state}
                    # {'address': 13, 'state': DigitalState.OFF_STATE}
                ])
                if self.program_state == ProgramState.PROG_RUNNING:
                    try:
                        Logger.info("Lidar OSSD (DI2) triggered. Stopping program.")
                        self.indy.stop_program()
                        self.indy.set_speed_ratio(100) # 복원: 프로그램 정지 후 속도 100으로 설정
                    except Exception as e:
                        Logger.error(f"Failed to stop program on DI2: {e}")
                return

            # 2순위: 경고 영역 (DI1) - 일시정지 상태 진입
            if is_di1_on:
                self.indy.set_do([
                    {'address': 10, 'state': DigitalState.ON_STATE}, # Red solid
                    {'address': 11, 'state': DigitalState.OFF_STATE},
                    {'address': 12, 'state': DigitalState.OFF_STATE},
                    {'address': 13, 'state': self.beep_state}
                    # {'address': 13, 'state': DigitalState.OFF_STATE}
                ])
                if self.program_state == ProgramState.PROG_RUNNING and not self.lidar_di1_paused:
                    try:
                        Logger.info("Lidar Protective (DI1) triggered. Pausing robot. Manual restart required.")
                        self.indy.set_speed_ratio(0)
                        bb.set("ui/state/lidar_popup", 1)
                        self.lidar_di1_paused = True
                    except Exception as e:
                        Logger.error(f"Failed to set speed to 0 on DI1: {e}")
                return

            # 3순위: 일시정지 대기 상태 (DI1에서 벗어났지만, 재시작은 안 한 상태)
            if self.lidar_di1_paused:
                # DI1에서 벗어났어도, 수동으로 재시작하기 전까지는 빨간불을 유지하며 대기
                self.indy.set_do([
                    {'address': 10, 'state': DigitalState.ON_STATE}, # Red solid
                    {'address': 11, 'state': DigitalState.OFF_STATE},
                    {'address': 12, 'state': DigitalState.OFF_STATE},
            ...
            if is_di0_on:
                self.indy.set_do([
                    {'address': 10, 'state': DigitalState.OFF_STATE},
                    {'address': 11, 'state': self.blinking_light_state}, # Yellow blinking
                    {'address': 12, 'state': DigitalState.OFF_STATE},
                    {'address': 13, 'state': DigitalState.OFF_STATE}
                ])
                if self.program_state == ProgramState.PROG_RUNNING:
                    ...
                    if current_speed != 30:
                        self.indy.set_speed_ratio(30)
                        Logger.info("Lidar Warning (DI0) triggered. Reducing speed to 30%.")
                return
            ...
            # 5순위: 모든 영역이 안전할 경우
                ...
                self.indy.set_do([
                    {'address': 10, 'state': DigitalState.OFF_STATE},
                    {'address': 11, 'state': DigitalState.OFF_STATE},
                    {'address': 12, 'state': DigitalState.ON_STATE}, # Green solid
                    {'address': 13, 'state': DigitalState.OFF_STATE}
                ])
                current_speed = self.indy.get_motion_data().get("speed_ratio", 100)
                if current_speed != 100:
                    self.indy.set_speed_ratio(100)
                    Logger.info("Lidar zones are clear. Resuming full speed.")

        except Exception as e:
            Logger.error(f"Error in handle_lidar_safety: {e}")
    ...
    def receive_data_from_bb(self):
        ...
        if bb.get("indy_command/play_program"):
            ...
            try:
                if self.is_sim_mode :
                    do_signals = self.indy.get_do()['signals']
                    is_di0_on = self.get_dio_channel(do_signals, 0) == 1
                    is_di1_on = self.get_dio_channel(do_signals, 1) == 1
                    is_di2_on = self.get_dio_channel(do_signals, 2) == 1
                else :
                    di_signals = self.indy.get_di()['signals']
                    is_di0_on = self.get_dio_channel(di_signals, 0) == 1
                    is_di1_on = self.get_dio_channel(di_signals, 1) == 1
