...
class RobotCommunication:
    ...
    def handle_lidar_safety(self):
        ...
        try:
            ...
            # Rising-edge 감지: 0 → 1로 변할 때만 처리
            if current_lidar_restart_state == 1 and self.last_lidar_restart_state == 0:
                Logger.info("Lidar restart button pressed (rising-edge detected).")
                
                # 안전 구역 확인: DI1과 DI0이 모두 꺼져있어야 안전
                if not is_di1_on and not is_di0_on:
                    # 안전 구역 → 재시작 허용
                    Logger.info("✅ Restart approved - Robot is in SAFE zone. Resuming operation.")
                    bb.set("ui/state/lidar_popup", 0)  # 팝업 닫기
                    self.indy.set_speed_ratio(100)      # 속도 복원
                    self.lidar_di1_paused = False       # 일시정지 플래그 해제
                elif is_di1_on:
                    # DI1 경고 영역 → 재시작 거부
                    Logger.warn("❌ Restart denied - Robot still in WARNING zone (DI1 active).")
                elif is_di0_on:
                    # DI0 감속 영역 → 재시작 거부
                    Logger.warn("❌ Restart denied - Robot still in DECELERATION zone (DI0 active).")
                
                # UI 명령 리셋 (버튼 상태 초기화 - 매우 중요!)
                bb.set("ui/reset/lidar_restart", True)
            
            # 이전 상태 저장 (다음 rising-edge 감지를 위해)
            self.last_lidar_restart_state = current_lidar_restart_state
            # ========================================
            ...
            # 2순위: 경고 영역 (DI1) - 정지 후 대기 (재시작 버튼 필요)
            elif is_di1_on:
                self.indy.set_do([
                    {'address': 10, 'state': DigitalState.ON_STATE},
                    {'address': 11, 'state': DigitalState.OFF_STATE},
                ...
                if self.program_state == ProgramState.PROG_RUNNING:
                    if not self.lidar_di1_paused:
                        try:
                            Logger.info("라이다 경고 영역(DI1) 침범. 로봇을 정지합니다. (재시작 버튼 필요)")
                            self.indy.set_speed_ratio(0)        # 속도 0으로 정지
                            bb.set("ui/state/lidar_popup", 1)   # 팝업 표시
                            self.lidar_di1_paused = True        # 일시정지 플래그 설정
                        except Exception as e:
                            Logger.error(f"속도 정지 실패: {e}")
                return
                
            # 3순위: 주의 영역 (DI0) - 속도 감속 (자동 복원)
            elif is_di0_on:
                self.indy.set_do([
                    {'address': 10, 'state': DigitalState.OFF_STATE},
                    {'address': 11, 'state': self.blinking_light_state},
                ...
                # lidar_di1_paused가 설정되어 있으면 DI0에서도 속도를 변경하지 않음
                if not self.lidar_di1_paused:
                    if self.program_state == ProgramState.PROG_RUNNING:
                        current_speed = self.indy.get_motion_data().get("speed_ratio", 100)
                        if current_speed != 30:
            ...
            # 모든 영역이 안전할 경우 (자동 복원은 DI0만 해당, DI1은 수동 재시작 필요)
            else:
                self.indy.set_do([
                    {'address': 10, 'state': DigitalState.OFF_STATE},
                    {'address': 11, 'state': DigitalState.OFF_STATE},
                ...
                # lidar_di1_paused가 설정되어 있으면 자동 복원하지 않음
                # (사용자가 재시작 버튼을 눌러야만 복원됨)
                if not self.lidar_di1_paused:
                    current_speed = self.indy.get_motion_data().get("speed_ratio", 100)
                    if current_speed != 100:
                        self.indy.set_speed_ratio(100)
