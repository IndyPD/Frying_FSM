...
def _log_strategy_activation(context: DryRecipeContext, strategy_name: str):
    ...
    if current != strategy_name:
        ...
        print(f"DEBUG: Strategy Activated -> {strategy_name} (Basket {context.basket_index})") # Added for debugging
...
class DryMoveToFryStrategy(Strategy):
    ...
    def operate(self, context: DryRecipeContext) -> DryRecipeFsmEvent:
        ...
        ''' Reset Menu: 바스켓 매뉴 취소 '''
        if context.cancel_basket():
            if not context.cancellation_in_progress:
                Logger.info(f"{get_time()}: [Basket {context.basket_index} FSM] Cancel acknowledged during MoveToFryer. Will complete motion and then exit.")
                context.cancellation_in_progress = True

        if bb.get("robot/state/worktarget") == context.basket_index :            
            putin_done = int(bb.get("int_var/putin_done/val"))            
            if putin_done :
                # If cancelling, don't start the cooking timer, but DO perform the initial shake.
                if not context.cancellation_in_progress:
                ...
                # 투입 후 흔들기 한번 실행 (This part seems necessary even during cancellation)
                # --- Debugging Start ---
        ...
        if context.basket_index == basket_idx:
            ...
            if context.cancellation_in_progress:
                Logger.info(f"{get_time()}: [Basket {context.basket_index} FSM] Canceled: Motion to fryer finished. Proceeding to next state to trigger exit.")
                # We still go to the next state (FRY), but the cancellation flag
                # will be checked there to immediately trigger the exit.
...
class DryFryStrategy(Strategy):
    ...
    def operate(self, context: DryRecipeContext) -> DryRecipeFsmEvent:
        ...
        # If we entered this state as part of a cancellation, exit immediately.
        if context.cancellation_in_progress:
            Logger.info(f"{get_time()}: [Basket {context.basket_index} FSM] In Fry state due to cancellation. Immediately moving out.")
...
class DryMoveFromFryStrategy(Strategy):
    ...
    def operate(self, context: DryRecipeContext) -> DryRecipeFsmEvent:
        ...
        # Always check for a cancellation request during this state.
        if int(bb.get(f"ui/command/fryer{context.fryer_index}/cancel")):
            if not context.cancellation_in_progress:
                Logger.info(f"{get_time()}: [Basket {context.basket_index} FSM] Cancel acknowledged during MoveFromFryer. Waiting for motion to complete.")
                context.cancellation_in_progress = True
        ...
        if context.basket_index == basket_idx:
            ...
            if context.cancellation_in_progress:
                # A cancel was requested during the move. Now that the move is complete,
                # clear the command flag to prevent it from affecting the next job.
                ...
                Logger.info(f"{get_time()}: [Basket {context.basket_index} FSM] Cleared cancel command after motion completion.")
