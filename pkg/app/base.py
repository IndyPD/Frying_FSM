...
class ModbusStyleCommunication(AppCommunication):
    ...
    def __init__(self, config_path, period_s=0.1, run_server=True):
        """ Init. thread """
        self.running = False
        self.thread = None
        ...
        self.server_info["address"] = global_config.get("robot_ip")
    ...
    def receive_data_from_app(self):
        ...
        for key, address in self.protocol['read_str']['forwarding'].items():
            str_data = self.client.get_string_with_id(address)
    ...
    def send_data_to_app(self):
        ...
        # Write float data if available
        if 'write_float' in self.protocol:
            for key, address in self.protocol['write_float']['forwarding'].items():
                if isinstance(address, int):
                    val = bb.get(key)
                    self.client.set_float(address, val)
                elif isinstance(address, list):
                    values = bb.get(key)
                    self.client.set_floats(address[0], values)
        ...
        for key, address in self.protocol['read_str_reset']['forwarding'].items():
            if bb.get(key):
                Logger.info(f"-------- {key} {address} {bb.get(key)}")
                result = self.client.set_string_with_id(address,'')
                Logger.info(f"---sdfsdfsdff=----------{result}")
                bb.set(key,False)
...
class ModbusStyleClientBase(ABC):
    ...
    ##
    # @brief read an array of int registers starting from an index
    # @param idx_ starting index of int value
    ...
    ##
    # @brief write a float register at index
    # @param idx index of float value
    # @param val float value to write
    @abstractmethod
    def set_float(self, idx, val):
        raise(NotImplementedError())

    ##
    # @brief write an array of float registers starting from an index
    # @param idx starting index of float value
    # @param val float values to write
    @abstractmethod
    def set_floats(self, idx, val):
        raise(NotImplementedError())

    ##
    # @brief read a float register at index
    # @param idx index of float value
    @abstractmethod
    ...
    ##
    # @brief read an array of float registers starting from an index
    # @param idx starting index of float value
    # @param count number of registers to read
    @abstractmethod
    def get_floats(self, idx, count) -> List[float]:
        raise(NotImplementedError())

    ##
    # @brief write string data with ID
    # @param idx index of string value
    # @param val string value to write
    @abstractmethod
    def set_string_with_id(self, idx, val) -> int:
        raise(NotImplementedError())

    ##
    # @brief read string data with ID
    # @param idx index of string value
    @abstractmethod
    def get_string_with_id(self, idx) -> str:
        raise(NotImplementedError())
