...
class GRPCGlobalVariableTaskServicer(template_pb2_grpc.GRPCGlobalVariableTaskServicer):
    ...
    def SetFloat(self, request, context):
        response = template_pb2.Empty()
        self.float_val[request.idx] = request.val
        return response

    def GetFloat(self, request, context):
        response = template_pb2.FloatVal()
        response.val = self.float_val[request.val]
        return response

    def SetFloats(self, request, context):
        for i in range(len(request.val)):
            self.float_val[request.idx+i] = request.val[i]
        response = template_pb2.Empty()
        return response

    def GetFloats(self, request, context):
        response = template_pb2.FloatVals()
        for i in range(request.val):
            response.val.append(self.float_val[request.idx+i])
        return response
    ...
    def run(self, port=GRPC_SERVER_PORT_DEFAULT):
        ...
        template_pb2_grpc.add_GRPCGlobalVariableTaskServicer_to_server(self, server_man)
